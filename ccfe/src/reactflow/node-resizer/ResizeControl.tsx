...
function ResizeControl({
  ...
  useEffect(() => {
    ...
    const startDrag = (event: ResizeDragEvent) => {
    ...
    const dragHandler = drag<HTMLDivElement, unknown>()
      .on("start", (event: ResizeDragEvent) => {
        const { nodeInternals, nodeDragThreshold } = store.getState()
      ...
      .on("drag", (event: ResizeDragEvent) => {
        const { nodeInternals, triggerNodeChanges, nodeDragThreshold } = store.getState()
        const { xSnapped, ySnapped } = getPointerPosition(event)

        if (!dragStarted.current) {
          const x = xSnapped - (startValues.current?.pointerX ?? 0)
          const y = ySnapped - (startValues.current?.pointerY ?? 0)
