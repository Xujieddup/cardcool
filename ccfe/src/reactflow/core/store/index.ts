  ...
  createWithEqualityFn<ReactFlowState>(
    (set, get) => ({
      ...
      updateNodeDimensions: (updates: NodeDimensionUpdate[]) => {
        ...
        const changes: NodeDimensionChange[] = updates.reduce<NodeDimensionChange[]>(
          (res, update) => {
        ...
        const nextFitViewOnInitDone =
          ...
          (fitViewOnInit &&
            !fitViewOnInitDone &&
            fitView(get, { initial: true, ...fitViewOnInitOptions }))
      ...
      updateNodePositions: (
        ...
        triggerNodeChanges(changes, true)
      },
      // 触发节点变更(拖拽节点边更新节点尺寸时 ignore 为 true)
      triggerNodeChanges: (changes: NodeChange[], ignore?: boolean) => {
        const {
          onNodesChange,
          nodeInternals,
          hasDefaultNodes,
      ...
      forceInitFitView: () => fitView(get, get().fitViewOnInitOptions, true),
    }),
    Object.is
  )
