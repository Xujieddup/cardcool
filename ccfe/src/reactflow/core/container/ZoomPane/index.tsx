...
import { clamp, isMacOs, isMacSafari } from "../../utils"
...
const wheelDeltaSafari = (event: any) => {
  return event.scale * 0.05
  return -event.deltaY * (event.deltaMode === 1 ? 0.05 : event.deltaMode ? 1 : 0.002) * 10
}
      ...
      if (panOnScroll && !zoomActivationKeyPressed && !userSelectionActive) {
        d3Selection.on(
          ...
          (event: any) => {
            ...
            // console.log("test", event.ctrlKey, zoomOnPinch, _isMacOs)
            ...
            const { onViewportChangeStart, onViewportChange, onViewportChangeEnd } =
              store.getState()
        ...
        if (isMacSafari() && zoomOnPinch) {
          const handleEvent = (event: any) => {
            ...
            const point = pointer(event)
            const zoom = gestureScaleRef.current * event.scale
            // @ts-ignore
            d3Zoom.scaleTo(d3Selection, zoom, point, event)
            // console.log("gesturechange", event, gestureScaleRef.current, point, pinchDelta, zoom)
          }
          d3Selection
            // 双指捏放时 chrome 会触发 wheel.zoom 事件，不会触发 gesture 事件，调用 wheeled 方法，从而调用 zoom()
            // .on("wheel.zoom", wheeled)
            // 双指捏放时 safari 不会触发 wheel.zoom 事件，会触发 gesture 事件，调用 gesture 方法，从而调用 zoom()
            // 每次缩放开始时调用一次
            .on("gesturestart", (e: any) => {
              e.preventDefault()
              gestureScaleRef.current = d3Selection.property("__zoom").k || 1
              // console.log("gesturestart")
            })
            .on("gesturechange", handleEvent)
            .on("gestureend", handleEvent)
        }
    ...
    if (d3Zoom) {
      d3Zoom.on("end", (event: D3ZoomEvent<HTMLDivElement, any>) => {
        ...
        if (
          ...
          viewChanged(prevTransform.current, event.transform)
        ) {
          const flowTransform = eventToFlowTransform(event.transform)
          prevTransform.current = flowTransform
