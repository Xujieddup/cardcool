  ...
  useEffect(() => {
    if (nodeRef?.current) {
        ...
        dragItems.current = dragItems.current.map((n) => {
          ...
          if (dragItems.current.length > 1 && nodeExtent && !n.extent) {
            ...
            adjustedNodeExtent[1][0] =
              n.positionAbsolute.x + (n.width ?? 0) - nodesBox.x2 + nodeExtent[1][0]

            adjustedNodeExtent[0][1] = n.positionAbsolute.y - nodesBox.y + nodeExtent[0][1]
            adjustedNodeExtent[1][1] =
              n.positionAbsolute.y + (n.height ?? 0) - nodesBox.y2 + nodeExtent[1][1]
          }
      ...
      const startDrag = (event: UseDragEvent) => {
        ...
        dragItems.current = getDragItems(nodeInternals, nodesDraggable, pointerPos, edges, nodeId)
        ...
        const dragHandler = drag()
          ...
          .on("end", (event: UseDragEvent) => {
            ...
            if (dragItems.current) {
              const { updateNodePositions, nodeInternals, onNodeDragStop, onSelectionDragStop } =
                store.getState()
