...
export default function useReactFlow<NodeData = any, EdgeData = any>(): ReactFlowInstance<
  NodeData,
  EdgeData
> {
  ...
  const deleteElements = useCallback<Instance.DeleteElements>(
    ({ nodes: nodesDeleted, edges: edgesDeleted }) => {
      ...
      const nodesToRemove = getNodes().reduce<Node[]>((res, node) => {
        const parentHit =
          !nodeIds.includes(node.id) && node.parentNode && res.find((n) => n.id === node.parentNode)
      ...
      if (nodesToRemove || initialHitEdges) {
        ...
        if (nodesToRemove.length > 0) {
          ...
          if (onNodesChange) {
            const nodeChanges: NodeChange[] = nodesToRemove.map((n) => ({
              id: n.id,
              type: "remove",
            }))
